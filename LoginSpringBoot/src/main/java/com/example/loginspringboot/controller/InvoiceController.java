...
public class InvoiceController
{
   /*@Autowired
    ...
    public ResponseEntity<Map<String,Object>> getAllInvoice(@RequestParam(defaultValue = "0")int page, @RequestParam(defaultValue = "10")int size, Authentication auth)
    {
        ...
        if (userDetails != null && userDetails.getAuthorities().stream().anyMatch(a -> a.getAuthority().equals("ROLE_USER")))
        {
            ...
            invoicePage = invoiceService.findAll(page,size);

            invoiceList = invoicePage.getContent();
            Map<String,Object> response = new HashMap<>();
            response.put("invoices", invoiceList);
            response.put("currentPage", invoicePage.getNumber());
            response.put("totalInvoice", invoicePage.getTotalElements());
            response.put("totalPages", invoicePage.getTotalPages());
            return new ResponseEntity<>(response, HttpStatus.OK);
        }
        else
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body((Map<String, Object>) new MessageResponse("You are not authorized to view this invoice."));
    }

    @PostMapping("/invoices/user")
    @PreAuthorize("hasRole('USER') or hasRole('MODERATOR') or hasRole('ADMIN')")
    ...
    @PutMapping("/invoices/{invoiceId}/user")
    @PreAuthorize("hasRole('USER') or hasRole('MODERATOR') or hasRole('ADMIN')")
    ...
    @DeleteMapping("/invoices/{invoiceId}/user")
    @PreAuthorize("hasRole('USER') or hasRole('MODERATOR') or hasRole('ADMIN')")
