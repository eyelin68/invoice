...
public class StuffController {
    ...
    @GetMapping("/stuffs/user")
    @PreAuthorize("hasRole('USER') or hasRole('MODERATOR') or hasRole('ADMIN')")
    public ResponseEntity<?> getAllStuffs(Authentication authentication,@RequestParam(defaultValue = "0")int page, @RequestParam(defaultValue = "10")int size)
    {
        ...
        if (userDetails != null && userDetails.getAuthorities().stream().anyMatch(auth -> auth.getAuthority().equals("ROLE_USER")))
        {
            ...
            stuffPage = stuffService.getAllStuffs(page,size);

            stuffList = stuffPage.getContent();
            Map<String,Object> response = new HashMap<>();
            response.put("stuffs", stuffList);
            response.put("currentPage", stuffPage.getNumber());
            response.put("totalStuffs", stuffPage.getTotalElements());
            response.put("totalPages", stuffPage.getTotalPages());
    ...
    @PostMapping("/invoice/{invoiceId}/stuffs/user")
    @PreAuthorize("hasRole('USER') or hasRole('MODERATOR') or hasRole('ADMIN')")
